...
We get an initial estimate of $k$ between $1.388$ and $1.470$ and $\psi$ between $\exp(1.975) = 7.201$ and $\exp(2.133) = 8.440$. Like the density plots the predictions are fairly similar. We compute simple confidence interval (based on the above table output) for the estimates of each imputation and plot them:
...
We note that the optimal value of $k$ using the AIC profile likelihood method is fairly close to the value of $k$ estimated by the linear model
```{r echo = FALSE}
pander(k_hat_lin)
```
...
pander(se_AIC)
```
and the lower and upper bounds of the 95% confidence interval are
...
```
The distribution is still very right skewed. This motivates the Gamma exponential dispersion model, which is typically used to fit positive continuous right skewed data. Consider now `Rain` plotted against `SOI`. 
...
```
There seems to be a positive relationship between `SOI` and `Rain`. The plot also indicates a possibility of non linear trends. The Gamma exponential dispersion model has a quadratic mean variance relationship, which also seems like a good fit from the plot above. There are a few observations with very large values of `Rain`, but none of which appear extreme enough to be considered outliers.

A typical link function choice for the Gamma model is the log link function. If we fit the model using the log link function, we fit the log of the mean of the response variables as a linear combination of the predictors. Other options include the identity link and the canonical link, which is the inverse function. A problem we have encountered when using the identity link is that for some parameters the model produces negative predictions, which is not possible for rain fall, and furthermore causes convergence issues. For this reason the identity link is disregarded. We fit two models: one with log link and one using the canonical link. 
...
```
The Gamma model with log link function seems to fit data reasonably well. We choose to proceed with the Gamma model with the log link rather than the inverse link function. 

## Should additional predictors be included?

We perform an LRT test to see whether or not we should include additional predictors. We consider to add the predictors `Phase` and `Year.` As mentioned in the EDA, `Phase` and `SOI` are correlated, and it could be problematic to include both in the model. But as discussed in the EDA, `Phase` contains information that `SOI` does not, meaning that the variable could potentially improve our predictions. 
...
```
The bootstrapped residual plots resemble the original residual plots well. This indicates that the fitted model is correct. A thing to note is that the few large residuals that we observed in the original residual plots are also present in the bootstrapped plots. 
...
We will now turn to the construction of confidence intervals for our model. First we will use nonparametric bootstrap to create a combinant based confidence interval for the model predictions as described on page 220. The code used for the pair sampling can be seen below
