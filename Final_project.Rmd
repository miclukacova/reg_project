...
The boxplot suggests that the rainfall is larger in phases 2 and 4, while it appears particularly low in phases 1 and 2. The plot display some correlation between `Phase` and `Rain` as there appear to be a difference in rainfall within the different phases. We proceed to investigate the correlation between `SOI` and `Phase`. Since both variables describe aspects of SOI we expect some correlation between them. We fit a simple linear regression model with `SOI` as the response variable and `Phase` as the predictor variable to test this:
...
Since the two predictors to some extent are collinear we will be careful, when we consider models with both covariates. It is worth noting though, that while `SOI` is a numeric variable containing exact values of SOI, `Phase` contains information on the 'direction' of the SOI, i.e. whether it is increasing, decreasing or constant. To sum up, both variables contain information that the other does not, so it could still be valuable to include them both in a model. We will go into further details on this matter in subsequent sections.

We finally investigate the correlation between `Year` and `Phase` with a boxplot:
...
ggplot(mYear,
  ...
  facet_wrap(~ variable, scale = "free_x", ncol = 2) + theme_bw()
```
The plot does not suggest any noteworthy co-linearity between `Year` and `Phase`. 
...
In the following part of the project we fit the Tweedie exponential dispersion model to the Eromanga rain data, and investigate if SOI phase is a good predictor of rainfall. First step is to estimate the nuisance parameter $k$ assumed to be between 1 and 2. 


## Estimating k with a linear regression model
We initially try to estimate $k$ using the relation $VY = \psi \mathcal{V}(\mu) = \psi \mu^k$. This implies the linear relation $\log (VY) = \log(\psi) + k \log(\mu)$ and we can therefore estimate $k$ by a linear regression of $\log (VY)$ on $\log(\mu)$. We compute the empirical variance and mean of the `Rain` variable within each SOI phase for each imputed data set and display the result for the first imputed data set:
...
We get an initial estimate of $k$ between $1.388$ and $1.470$ and $\psi$ between $\exp(1.975) = 7.201$ and $\exp(2.133) = 8.440$. Like the density plots of imputed data sets, the estimates for the different imputed data sets are fairly similar. We compute confidence interval of the form $\hat k \pm1.96 \text{se}(\hat k)$ (based on the standard deviation in the above table output) for the estimates of each imputation and plot them:
...
```{r echo = F}
k_psi_confint <- rbind(confint(lm.fit), exp(c(confint(lm.fit)[1,1], confint(lm.fit)[1,2])))
rownames(k_psi_confint) <- c("log psi", "k","psi")
pander(k_psi_confint)
```
...
We see no warning signs from the plot so it seems fair to assume, that the estimated values of $k$ and $\psi$ are decent initial estimates of the true values.
...
pander(psi_hat_lin)
```
obtained from the linear regression, but still well within the statistical uncertainty as shown from the profile-likelihood confidence interval.
...
With the two estimates of $\psi$ from the previous exercises, the estimate of $k$ obtained from the linear regression and the empirical mean of our data we can calculate the probability of zero rain in July with the above formula. In addition we calculate the empirical probability of zero rain in July and compare the three estimates:

```{r}
mu_hat <- mean(Rain.data.comp$Rain)
emp_prob_zero <- mean(Rain.data.comp$Rain == 0)

pander(c("Psi estimate from linear model" 
         = exp(-mu_hat^(2 - k_hat_lin)/(psi_hat_lin*(2 - k_hat_lin))), 
         "Psi estimate from Tweedie model" 
         = exp(-mu_hat^(2 - k_hat_lin)/(psi_hat_tweedie.fit*(2 - k_hat_lin))),
         "Empirical probability of zero rain in July" = emp_prob_zero))
```
All three estimates are very close to each other, which suggests that the fitted models are reasonable.


## Determining k by minimizing AIC
We now estimate $k$ by minimizing the Akaike Information Criterion (AIC) with a profile likelihood of a model with SOI phase as explanatory variable. We start by defining a general profile likelihood function that takes inputs a formula, a family, a data set and an evaluation metric that we wish to optimize.
...
We then define the specific profile likelihood that minimizes the AIC for different values of $k$ of a Tweedie exponential model with rainfall as response and SOI phase as covariate.  That is, we specify `form = Rain ~ Phase`, `family = tweedie(var.power = k, link.power = 0)`, `data = Rain.data.comp` and `eval = AICtweedie`:
...
We use `optimize()` to minimize the AIC and find the optimal value of $k$. We only check values of $k$ between $1.05$ and $1.95$ to avoid numerical instability. The optimal value of $k$ is
```{r}
...
pander(k_hat_lin)
```
- the value of $k$ estimated by the linear model. To ensure, that we have indeed found a minimum for $1<k<2$ we plot the AIC against $k$.
...
As before we initially fit a Tweedie model to the data using the value of $k$ found by minimizing the AIC profile likelihood.
```{r}
...
```
is also a bit smaller than the estimate
```{r echo = FALSE}
pander(psi_hat_tweedie.fit)
```
from the previous Tweedie model. Apart from that the results are identical to the previous model and the interpretation is the same.
...
pander(c("Psi estimate from linear model" 
         ...
         "Empirical probability of zero rain in July" = emp_prob_zero))
```
Again the estimates are similar to the estimates obtained from the previous estimate of $k$. To sum up, both ways of estimating $k$ produces very similar results.
...
We then plot the residuals against the fitted values, first for Tweedie model using the linear model estimate of $k$:
...
```
We see that the plots by and large resemble the residual plots of the original model. This supports the initial diagnostic plots and conclusion - that the model seem to be a reasonable fit. We repeat the procedure for the Tweedie model fitted with the AIC profile likelihood estimate of $k$.
...
## Conclusion
The analysis indicates an association between rainfall and SOI phase, a relationship initially suggested by the boxplot of rainfall and SOI phase in the EDA and later supported by the fitted models in the subsequent analysis. Based on the model diagnostics, both fitted models seem to adequately capture the mean and variance structure of the data, allowing us to reasonably trust the model conclusions. The various estimates of $k$ and $\psi$ were all close and considering the profile-likelihood confidence interval of $k$ and $\log(\psi)$ all estimates were within a statistical margin of error.

It therefore appears, that using SOI phase as the only predictor of rainfall is a reasonable approach. However, as mentioned there could be an issue with the modest amount of observations in Phase 3, which makes it difficult to estimate the effect of this phase. Therefore it may be useful to construct a model using `SOI` instead of `Phase`, which we will look into in a coming part of the analysis.
...
```
respectively. We note that this confidence interval is slightly narrower than the profile likelihood confidence interval of $k$ obtained from the the linear model. Both confidence intervals contain the two estimates of $k$ and we therefore conclude that the two estimates of $k$ are not significantly different.
...
We consider to include a non-linear effect of `SOI`. In particular we explore the inclusion of a natural cubic splines with 2,3,4,5 or 6 degrees of freedom:
...
From the plots we see no clear evidence against the weak model assumptions GA1 and GA2. The residuals seem to be randomly scattered around zero with constant variance. There are a few large residuals, but they are not alarming. As the PP-plot displays, the stronger distributional assumption GA3 seems to be slightly off, which implies that inference relying on distributional results should be avoided or employed with consideration. 
...
The fitted model predicts that rainfall is increasing as a function of `SOI.` The slope of the model is largest for values of `SOI` between $-10$ and $10$. For `SOI` values that are larger or smaller than this, the model predictions are more constant. 
