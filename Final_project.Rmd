...
We conclude this part of the analysis with parametric bootstrap to estimate the sampling distribution of $k$. We use parameters estimated by a Tweedie model to simulate new data from a Tweedie distribution and fit a new Tweedie model to the simulated data to estimate $k$. We repeat this process a number of times to estimate the sampling distribution of $k$. To carry out the parametric bootstrap we need the strong distributional assumptions GA3 and A5. We have not formally tested the validity of these assumptions, so further work should look into these assumptions. For now we should keep in mind, that the parametric bootstrap may result in too narrow confidence intervals.

We try to estimate the sampling distribution of $k$ using both the AIC method and the linear model method as previous. Due to the small number of observations in phase 3 there is a high probability, that some bootstrap samples will contain only zeros for phase 3. This will cause the linear model method to fail, as the mean and variance in this group will be zero and taking the log of these values produces NAs. For the AIC method the fitted model used in the profile-likelihood will fail to converge for these bootstrap samples. We therefore need to guard ourselves against these cases. We have decided to add a small random value to the zeros in a group, in case they are all zeros. This will likely introduce a bit of bias to the results but alternatively we can not use the approach at all or the estimates we get comes from an algorithm that has not converged, hence the estimates are meaningless.

We first use the linear model method:

```{r}
set.seed(10102024)
B <- 1000
parametric_k_LIN <- numeric(B)
rain_data_LIN_sample <- Rain.data.comp

for (b in 1:B){
  # Simulating tweedie random variables using the estimates linmu_hats from
  # the tweedie model with the linear model estimate of k
  rain_data_LIN_sample$Rain <- rTweedie(linmu_hats, 
                                        psi_hat_tweedie.fit, 
                                        p = k_hat_lin)
  ...
  # Add small (random) constant to phases where sum of Rain is zero to avoid log(0)
  rain_data_LIN_sample <- rain_data_LIN_sample %>%
...
```{r warning=FALSE, echo = FALSE, message = FALSE, fig.width = 10, fig.height = 6, fig.align = 'center'}
violin_df <- data.frame(k = c(parametric_k_LIN, parametric_k_AIC),
                        method = as.factor(rep(c("Linear model", "AIC"), each = B)))

ggplot(violin_df, aes(x = method, y = k)) +
  geom_violin(fill = I(gray(0.8))) + xlab("") +
  facet_wrap(~ method, scale = "free_x", ncol = 2) + theme_bw() +
  geom_hline(yintercept = k_hat_lin, color = "blue", linetype = "dashed") +
  geom_hline(yintercept = k_hat_AIC, color = "red", linetype = "dashed") +
  annotate("text", x = 1, y = k_hat_lin - 0.08, 
           label = "Linear model estimate of k", 
           color = "blue", vjust = -0.5) +
  annotate("text", x = 1, y = k_hat_AIC + 0.03, 
           label = "AIC estimate of k", color = "red", vjust = -0.5)
```

It appears that the AIC method is more robust than the linear model method. The AIC method has a smaller variance and is closer to the value of $k$ estimated on the real data. The spike on both plots come from the few bootstrap samples where phase 3 has only zero observations. We use the parametric AIC bootstrap to estimate the standard error of the AIC estimate of $k$:
