...
Both models seem to fit the data reasonably well. There is no evidence of overfitting so we assume that the training error would generalize well. Based on this, we choose to proceed with the the log link gamma model rather than the inverse link model.
...
```
According to the LRT test there is not evidence in data that suggest that the additional predictors should be added to the model. We note that the likelihood ratio test hinges on the GA3 assumption. If this assumption is violated, the test may not be reliable. However based on the weak correlation and the considerations regarding `SOI`and `Phase` together with the results of the test it seems very unlikely that including additional predictors will improve the predictive strentgth of the model. We therefore choose to proceed with the model that only includes `SOI` as a predictor.
...
We will further construct a confidence intervals of the form

$$
\hat{f} \pm 1.96 \hat{\text{se}} 
$$

We will use the analytical estimates of the standard errors of the model predictions. Note that this interval will be symmetric around the point estimate, and that the interval is based on distributional assumptions regarding the $Z$-score. However, it could still be interesting to compare this approach with the bootstrap approach.
...
We see that the two confidence interval are narrow for the `SOI` values where we have many observations and wide for the `SOI` values for which we have few observations. This is to be expected. A thing to point out is the asymmetry that is present in the bootstrap combinant based confidence interval, indicating a certain asymmetry of the distribution of the model predictions. Apart from this, the two confidence intervals resemble each other quite a lot. 
...
The model is well fitted to the data and the residuals show no evidence against the model assumptions. However the PP-plot showed that the GA3 assumption might not be completely satisfied. 

According to the model, `SOI` is a significant predictor of `Rain`. The model predicts that `Rain` is increasing as a function of `SOI`. We have constructed confidence intervals for the model predictions, which show that the model is most certain for `SOI` values for which we have many observations, and less certain for `SOI` values where we have few observations. The bootstrap combinant base confidence interval as well as the analytical standard error based confidence interval show similar results.
