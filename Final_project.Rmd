...
We get an initial estimate of $k$ between $1.388$ and $1.470$ and $\psi$ between $\exp(1.975) = 7.201$ and $\exp(2.133) = 8.440$. Like the density plots the estimates are fairly similar. We compute simple confidence interval (based on the above table output) for the estimates of each imputation and plot them:
...
```
The estimate of $k = 2.198$ suggest that the Gamma model could be a reasonable fit. This method of estimating $k$ is of course only valid if the Tweedie model is areasonable fit for the data. 
