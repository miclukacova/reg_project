...
In the following part of the project we seek to fit the Tweedie exponential dispersion model to the Eromanga rain data, and predict rainfall as a function of the SOI phase. To fit a Tweedie model to we need to estimate the nuisance parameter $k$ assumed to be between 1 and 2. 
...
We see no warning signs from the plot so is seems fair to assume, that the estimated values of $k$ and $\psi$ are descent initial estimates of the true values.
...
Note that Phase1 is the reference phase (Intercept). The model suggests that the rainfall for Phase 1 is significantly different from 0 with a point estimate of the average rainfall of 
```{r echo = FALSE}
