...
exp(-mu_hat^(2 - k_hat)/(psi_hat_model*(2 - k_hat)))
```
...
k_hat_optim_obj <- optimize(calculate_AIC, lower = 1.1, upper = 1.9)$objective
k_hat_optim
```
...
```{r}
k_plot_df <- data.frame(k = seq(1.05, 1.95, by = 0.001), AIC = sapply(seq(1.05, 1.95, by = 0.001), FUN = calculate_AIC))
